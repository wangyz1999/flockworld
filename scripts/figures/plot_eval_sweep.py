...
Usage:
  uv run python -m scripts.figures.plot_eval_sweep --results-dir <dir> --plots-dir <dir> --tables-dir <dir>
"""
...
# labels mirror modeling/cli/compare_experiments.py's console-table convention (metric name + direction)
QUALITY_LABEL = {
...
STREAMING_ORDER = [
    ...
    # see modeling/cli/compare_experiments.py --extra-single-agent-*.
...
def load_entries(group_dir: Path) -> list[dict]:
    ...
    for p in sorted(group_dir.glob("*.json")):
        ...
        if "experiment" not in entry:
            # e.g. modeling/cli/compare_experiments.py's combined comparison.json living alongside the
