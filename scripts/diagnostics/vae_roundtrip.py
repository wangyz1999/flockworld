...
modeling/cli/precompute_latents.py), encodes with the frozen VAE, decodes back, writes the
...
Usage:
  uv run python -m scripts.diagnostics.vae_roundtrip
  uv run python -m scripts.diagnostics.vae_roundtrip --videos data/recording/20260702160453/video_a3/00000.mp4
"""
...
from modeling.eval.common import write_mp4
...
def load_clip(path: str, img_size=(128, 128)) -> torch.Tensor:
    """video -> (1,3,T,H,W) float32 in [-1,1], exactly like modeling/cli/precompute_latents.py."""
