...
for Setup 1 (no color, no bg gradient). Mirrors eval_flock_multi.py's
...
pair_consistency.py can't compute here (no hue to key off).
    ...
    uv run python eval_heading_identity.py --episodes 6 --seconds 10
...
from eval_flock_multi import _load_model, _rollout_model, _rollout_single
