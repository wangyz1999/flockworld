...
Run from the repository root::
    ...
    uv run python -m modeling.cli.train_vae
    uv run python -m modeling.cli.train_vae trainer.max_epochs=1 dataloader.batch_size=1
...
def load_cfg(config_path: str | None, overrides: list[str]):
    path = Path(config_path) if config_path else Path(__file__).resolve().parents[2] / "config" / "train_vae.yaml"
