...
Usage:
    uv run python -m modeling.cli.precompute_latents --config config/train_flockdit_latent.yaml [--limit N]
"""
...
def main():
    ...
    if shard is None and skipped == 0:
        ...
        print(f"done -> {cache_dir} ({why}). Now run once to write stats.pt:\n"
              f"  uv run python -m modeling.cli.precompute_latents --config {args.config} --stats-only "
