#!/bin/bash
# Full-fidelity (6 episodes / 10s / 50 Euler steps -- modeling/cli/eval_flock_multi.py's own defaults for
...
# (see modeling/cli/compare_experiments.py / modeling/cli/eval_flock_multi.py --save-csv), so future plotting never
# needs a rerun.
#
...
# modeling/cli/compare_experiments.py call (shares the held-out dataset build across columns), plus
...
run streaming_arch_full "$LOGS/streaming_arch_full.log" \
  uv run python -m modeling.cli.compare_experiments \
...
run_agentcount() {
  ...
  run "agentcount_$tag" "$LOGS/agentcount_$tag.log" \
    uv run python -m modeling.cli.eval_flock_multi --mode metrics \
      --episodes "$EPISODES" --seconds "$SECONDS_" --steps "$STEPS" \
...
# can't share modeling/cli/compare_experiments.py's single shared-VAE table, same as before).
